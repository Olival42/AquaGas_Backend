...
public sealed class GetAllPlans : IGetAllPlans
{
    ...
    public async Task<Result<List<PlanResponse>>> Execute()
    {
        var plans =
        ...
        foreach (var plan in plans)
        {
            ...
            outputs.Add(new PlanResponse
            {
                Id = plan.Id,
                CustomerId = customer?.Id ?? Guid.Empty,
                CustomerName = customer?.Name.Value ?? string.Empty,
                Document = customer?.Document.Value ?? string.Empty,
                EmployeeId = employee?.Id ?? Guid.Empty,
                EmployeeName = employee?.Name.Value ?? string.Empty,
                Cycle = plan.Cycle,
                Status = plan.Status,
        ...
        return Result<List<PlanResponse>>
            .Success(outputs);
    }
}