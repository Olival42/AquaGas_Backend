...
public class GetByIdTests
{
    ...
    private static EmployeeEntity CreateEmployee()
        => new(
            EmployeeName.Create("Employee Name").Value!,
    ...
    public async Task Should_Return_NotFound_When_Plan_Does_Not_Exist()
    {
        ...
        result.Errors.First().Code.Should().Be("NOT_FOUND");
    }

    [Fact]
    public async Task Should_Return_NotFound_When_Customer_Not_Found()
    {
        var plan = CreatePlan();
    ...
    [Fact]
    public async Task Should_Return_NotFound_When_Employee_Not_Found()
    {
        var plan = CreatePlan();
    ...
    [Fact]
    public async Task Should_Return_Success_With_Full_Plan()
    {
        var plan = CreatePlan();
        var customer = CreateCustomer();
        ...
        result.Value!.Id.Should().Be(plan.Id);
        result.Value.CustomerName.Should().Be(customer.Name.Value);
        result.Value.EmployeeName.Should().Be(employee.Name.Value);
        result.Value.Items.Should().HaveCount(1);
        result.Value.Deliveries.Should().HaveCount(1);
        result.Value.Billings.Should().HaveCount(1);
        result.Value.Penalties.Should().BeEmpty();
    }

    [Fact]
    public async Task Should_Update_Statuses_And_Persist_When_Changes_Happen()
    {
        var plan = CreatePlan();
        var customer = CreateCustomer();
        var employee = CreateEmployee();
